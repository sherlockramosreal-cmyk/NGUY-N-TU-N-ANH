...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans antialiased p-4 sm:p-6">
  <div class="max-w-4xl mx-auto space-y-6">
    <!-- Header -->
    <header class="p-5 rounded-2xl bg-slate-900 border border-slate-800 flex flex-wrap items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-slate-400">Sandbox Preview (Isolated IFrame)</span>
        </div>
        <h1 class="text-xl font-bold text-white mt-1">${topic}</h1>
      </div>
      <div class="flex items-center gap-3">
        <div class="px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-xs font-mono">
    ...
    <div class="flex flex-wrap gap-2 border-b border-slate-800 pb-2">
      <button onclick="switchTab('import')" id="tabImportBtn" class="px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white flex items-center gap-1.5">
        📥 Form Nạp Bài Học Mới
      </button>
      <button onclick="switchTab('flashcard')" id="tabFlashcardBtn" class="px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5">
        🎴 3D Flashcard (<span id="fcCountBadge">4</span>)
      </button>
      <button onclick="switchTab('quiz')" id="tabQuizBtn" class="px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5">
        ⚡ Trắc nghiệm phản xạ
      </button>
      <button onclick="switchTab('theory')" id="tabTheoryBtn" class="px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5">
        📖 Lý thuyết trọng tâm
      </button>
    </div>

    <!-- Section 0: Form Nạp Bài Học Mới & Bóc tách JavaScript -->
    <section id="sectionImport" class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-800">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">Dán danh sách từ vựng theo dạng <code>từ : nghĩa</code> hoặc <code>từ - nghĩa</code>. Ứng dụng tự động bóc tách và nạp vào Flashcard & Quiz ngay!</p>
        </div>
        <div class="flex items-center gap-2">
          <button onclick="loadColonSample()" class="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition">
            📋 Mẫu dấu ":"
          </button>
          <button onclick="loadDashSample()" class="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition">
            📋 Mẫu dấu "-"
          </button>
        </div>
      </div>
...
renewable energy - năng lượng tái tạo" class="w-full p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none leading-relaxed transition-all"></textarea>
    ...
    <!-- Section 1: Theory -->
    <section id="sectionTheory" class="hidden p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
      <h2 class="text-base font-bold text-indigo-400 flex items-center gap-2">
        <span>💡 Tóm tắt Lý thuyết & Quy tắc Ngữ pháp</span>
      </h2>
      <div class="p-4 rounded-xl bg-slate-950/80 border border-slate-800 text-xs leading-relaxed text-slate-300">
        ${theory}
      </div>
    </section>

    <!-- Section 2: Flashcard -->
    <section id="sectionFlashcard" class="hidden p-6 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col items-center space-y-5">
      <div class="flex items-center justify-between w-full max-w-md text-xs text-slate-400">
        <span>Thẻ: <strong id="fcIdxDisplay" class="text-white">1</strong> / <span id="fcTotalDisplay">4</span></span>
        <button onclick="shuffleCards()" class="hover:text-indigo-400 transition font-medium">🔀 Xáo bài</button>
      </div>

      <div class="w-full max-w-md card-flip h-64 cursor-pointer" onclick="toggleCard()">
        <div id="flashcardInner" class="card-inner relative w-full h-full rounded-2xl bg-slate-950 border border-slate-800 p-6 flex flex-col items-center justify-center text-center shadow-xl">
          <div class="card-front absolute inset-0 p-6 flex flex-col items-center justify-center">
            <span id="fcPos" class="text-xs font-bold px-2.5 py-0.5 rounded-md bg-indigo-950 text-indigo-400 mb-2 uppercase border border-indigo-800/60">ADJECTIVE</span>
            <h3 id="fcWord" class="text-2xl font-black text-white">sustainable</h3>
            <p id="fcIpa" class="text-xs text-slate-400 font-mono mt-1">/səˈsteɪ.nə.bəl/</p>
            <p class="text-[11px] text-indigo-400 mt-4">👉 Click hoặc chạm để lật mặt sau</p>
          </div>
          <div class="card-back absolute inset-0 p-6 flex flex-col justify-between bg-slate-900 rounded-2xl border border-indigo-900/50">
            <div>
              <p id="fcDefVi" class="text-sm font-bold text-emerald-400">bền vững, thân thiện với môi trường</p>
              <p id="fcDefEn" class="text-xs text-slate-300 italic mt-1">"able to continue without causing damage to the environment"</p>
            </div>
            <div id="fcExample" class="p-2.5 rounded-lg bg-slate-950 text-[11px] text-slate-300 border border-slate-800 text-left">
      ...
      <div class="flex items-center gap-3">
        <button onclick="prevCard()" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition">
          ◀ Trước
        </button>
        <button onclick="speakCurrentWord()" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold flex items-center gap-2 transition shadow-md shadow-indigo-600/20">
          🔊 Phát âm (US)
        </button>
        <button onclick="nextCard()" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition">
          Tiếp ▶
        </button>
      </div>
    </section>
    ...
    <!-- Section 3: Quiz -->
    <section id="sectionQuiz" class="hidden p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
      <div class="flex items-center justify-between">
        <h2 class="text-sm font-bold text-indigo-400">Câu hỏi ôn tập phản xạ:</h2>
        <button onclick="generateNextQuiz()" class="text-xs text-slate-400 hover:text-white underline">Đổi câu hỏi khác</button>
      </div>
      <p id="quizQuestionPrompt" class="text-sm text-white font-medium">Từ nào sau đây mang ý nghĩa <span id="quizTargetDef" class="text-emerald-400 font-bold">"bền vững, thân thiện với môi trường"</span>?</p>
  ...
  <script>
    ...
    function switchTab(tab) {
      ...
      document.getElementById('tabImportBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5';
      document.getElementById('tabTheoryBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5';
      document.getElementById('tabFlashcardBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5';
      document.getElementById('tabQuizBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-slate-400 hover:text-white flex items-center gap-1.5';

      if (tab === 'import') {
        document.getElementById('sectionImport').classList.remove('hidden');
        document.getElementById('tabImportBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white flex items-center gap-1.5';
      } else if (tab === 'theory') {
        document.getElementById('sectionTheory').classList.remove('hidden');
        document.getElementById('tabTheoryBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white flex items-center gap-1.5';
      } else if (tab === 'flashcard') {
        document.getElementById('sectionFlashcard').classList.remove('hidden');
        document.getElementById('tabFlashcardBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white flex items-center gap-1.5';
        renderCurrentCard();
      } else if (tab === 'quiz') {
        document.getElementById('sectionQuiz').classList.remove('hidden');
        document.getElementById('tabQuizBtn').className = 'px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white flex items-center gap-1.5';
        generateNextQuiz();
      }
    }
...
collaboration - sự cộng tác : Group work fosters effective collaboration.\`;
    ...
    function generateNextQuiz() {
      ...
      options.forEach((opt, idx) => {
        ...
        btn.className = 'p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-left hover:border-slate-700 transition cursor-pointer';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">
                  Thực thi An Toàn
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Chủ đề: <span className="font-medium text-slate-700 dark:text-slate-300">{config.lessonTopic}</span> • Khối: {config.gradeLevel.toUpperCase()}
              </p>
            </div>
          </div>
        ...
        {modalMode === 'react_demo' ? (
          <>
            {/* Tab Navigation */}
            <div className="px-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
            ...
            {/* Tab Content */}
            <div className="flex-1 p-6 overflow-y-auto bg-slate-50/50 dark:bg-slate-950/30 flex flex-col items-center justify-center min-h-[400px]">
              {activeTab === 'flashcard' && (
                <div className="w-full max-w-lg flex flex-col items-center space-y-4">
                  <div className="w-full flex justify-between text-xs text-slate-400 font-medium px-2">
                  ...
                  >
                    <div
                      className={`relative w-full h-full duration-500 rounded-3xl transition-transform [transform-style:preserve-3d] shadow-xl border border-slate-200/80 dark:border-slate-800 ${
                        isFlipped ? '[transform:rotateY(180deg)]' : ''
                      }`}
                    >
                      {/* FRONT */}
                      <div className="absolute inset-0 w-full h-full rounded-3xl bg-white dark:bg-slate-900 p-8 flex flex-col items-center justify-center text-center [backface-visibility:hidden]">
                        <span className="text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 dark:bg-indigo-950/80 dark:text-indigo-400 mb-3 border border-indigo-100 dark:border-indigo-900">
                          {currentWord.type}
                        </span>
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-md transition hover:scale-105 active:scale-95"
                        >
                      ...
                      {/* BACK */}
                      <div className="absolute inset-0 w-full h-full rounded-3xl bg-slate-900 text-white p-7 flex flex-col justify-between [transform:rotateY(180deg)] [backface-visibility:hidden] border border-slate-700">
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-indigo-400 font-bold uppercase tracking-wider">{currentWord.word}</span>
                            <span className="text-[10px] text-slate-400">Nghĩa & Ngữ cảnh</span>
                          </div>
                          ...
                          <div className="bg-slate-800/80 rounded-xl p-2.5 border border-slate-700/60 mb-2">
                            <span className="text-[10px] uppercase font-bold text-amber-400 block mb-1">Collocations:</span>
                            <div className="flex flex-wrap gap-1.5">
                              {currentWord.collocations.map((c, i) => (
                                <span key={i} className="text-[11px] bg-slate-950 px-2 py-0.5 rounded-lg text-slate-300 border border-slate-700">
                                  {c}
                                </span>
                              ))}
                            </div>
                        ...
                        <div className="text-xs bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-800/40">
                          <p className="text-slate-200 font-medium">{currentWord.example_en}</p>
                          <p className="text-slate-400 text-[11px] mt-0.5">{currentWord.example_vi}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between w-full pt-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 transition"
                    >
                      <RotateCw className="w-3.5 h-3.5" />
                      Lật thẻ (Flip)
                    </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow transition"
                    >
                      Thẻ tiếp theo ➔
                    </button>
                  </div>
              ...
              {activeTab === 'quiz' && (
                <div className="w-full max-w-lg space-y-4">
                  <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md">
                    <span className="text-[11px] font-bold uppercase text-indigo-600 dark:text-indigo-400 tracking-wider">
                      Active Recall • Trắc nghiệm phản xạ
                    </span>
                    <h3 className="text-base font-bold text-slate-900 dark:text-white mt-1 mb-4">
                      Từ vựng nào mang ý nghĩa: <span className="text-indigo-600 dark:text-indigo-400">"bền vững, thân thiện với môi trường"</span>?
                    </h3>
                    ...
                    <div className="space-y-2.5">
                      {[
                        ...
                        let btnStyle = 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200';
                        if (quizSubmitted) {
                          if (opt.correct) {
                            btnStyle = 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-bold';
                          } else if (isSelected && !opt.correct) {
                            btnStyle = 'border-rose-500 bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300';
                          }
                        } else if (isSelected) {
                          btnStyle = 'border-indigo-600 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-semibold';
                        }
                    ...
                    {!quizSubmitted ? (
                      <button
                        ...
                        className="w-full mt-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs shadow transition"
                      >
                        Kiểm tra đáp án
                      </button>
                    ) : (
                      <div className="mt-4 p-3 rounded-xl bg-indigo-50/80 dark:bg-slate-800/90 border border-indigo-100 dark:border-slate-700 text-xs">
                ...
                <div className="w-full max-w-md space-y-4 text-center">
                  <div className="flex justify-between items-center bg-slate-900 text-white p-3 rounded-2xl">
                    <div className="text-xs">
                      <span className="text-slate-400">Thời gian:</span> <span className="font-mono font-bold text-amber-400">{timeLeft}s</span>
                    </div>
                    <div className="text-xs">
                      <span className="text-slate-400">Combo:</span> <span className="font-bold text-emerald-400">x{combo}</span>
                    </div>
                    <div className="text-xs">
                      <span className="text-slate-400">Điểm:</span> <span className="font-bold text-indigo-400">{score}</span>
                    </div>
                  </div>
                  ...
                  {speedRunning ? (
                    <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-5">
                      <span className="text-[10px] uppercase font-bold text-slate-400">Cặp từ ngữ này ĐÚNG hay SAI?</span>
                      
                      <div className="py-2">
                        <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">
                          {speedQuestion.word}
                        </h2>
                        <div className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 p-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-900">
                          = {speedQuestion.def}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className="p-4 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow-lg hover:scale-105 active:scale-95 transition flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                          ...
                          className="p-4 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-sm shadow-lg hover:scale-105 active:scale-95 transition flex items-center justify-center gap-2"
                        >
                    ...
                    <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
                      ...
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">Trò chơi kết thúc!</h3>
                      <p className="text-sm text-slate-500">Điểm số phản xạ của bạn: <span className="font-bold text-indigo-600">{score} điểm</span></p>
                      <button
                        onClick={() => {
                          setScore(0);
                          setCombo(0);
                        ...
                        className="px-6 py-2.5 rounded-2xl bg-indigo-600 text-white font-bold text-xs shadow-lg hover:bg-indigo-700 transition flex items-center justify-center gap-2 mx-auto"
                      >
          ...
          /* Sandbox Runner Mode */
          <div className="flex-1 flex flex-col md:flex-row h-full min-h-[480px] overflow-hidden bg-slate-950">
            {/* Left/Top Editor & Controls */}
            <div className="w-full md:w-5/12 p-4 border-r border-slate-800 flex flex-col bg-slate-900/60">
              <div className="flex items-center justify-between mb-2">
              ...
              <textarea
                ...
                className="flex-1 w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 font-mono text-[11px] leading-relaxed resize-none focus:outline-hidden focus:ring-1 focus:ring-emerald-500"
              />
            ...
            <div className="w-full md:w-7/12 p-4 flex flex-col bg-slate-950">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
              ...
              <div className="flex-1 w-full rounded-2xl overflow-hidden border border-slate-800 bg-slate-900 shadow-inner flex flex-col">
                <iframe
                  key={iframeKey}
                  title="HTML Sandbox Runner"
                  sandbox="allow-scripts allow-same-origin"
                  srcDoc={customHtmlCode}
                  className="w-full h-full min-h-[420px] bg-white border-0"
                />
              </div>
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center text-xs text-slate-500 dark:text-slate-400">
          <span>✨ Ứng dụng mô phỏng các cơ chế sinh mã từ Master Prompt Tự Học</span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold transition"
          >
            Đóng cửa sổ
          </button>
        </div>
